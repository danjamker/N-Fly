...
def precision(human, computer):
    ...
    a = len(setUnion(human ,computer))
    b = len(setDifrence(computer ,setUnion(human, computer)))
...
def recall(human, computer):
    ...
    c = len(setDifrence(human ,setUnion(human, computer)))
...
if __name__ == '__main__':
    getcontext().prec = 50
    try:
        db = MySQL.Connect("localhost", "root", "root", "evaluation2")
        cur = db.cursor()
        
        cur.execute("SELECT * FROM evaluation2.setup")
        ...
        for setup in data:
            ...
            cur.execute("INSERT INTO `evaluation2`.`test` (`date`, `idsetup`) VALUES (NOW(), "+str(setup[0])+")")
            ...
            for page in datawp:
                ...
                sql = "INSERT INTO `evaluation2`.`test_webpage` (`runtime`, `idwebpage`, `idtest`, `prosision`, `fmeasure`, `recall`, `date`) VALUES (%s ,%s ,%s ,%s ,%s ,%s , NOW())" % (t_diffrence,page[0],test[0],p,f,r,)
                cur.execute(sql)
                cur.execute("SELECT * FROM test_webpage ORDER BY idtest DESC LIMIT 1")
                test_webpage = cur.fetchone()
                
                for word in keywords:
                    sql = "INSERT INTO `evaluation2`.`test_keyword` (`keyword`, `idwebpage`) VALUES ('"+word+"',"+str(test_webpage[0])+")"
                    cur.execute(sql)
